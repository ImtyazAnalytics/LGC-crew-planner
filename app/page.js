...
export default function Home() {
  ...
  const [teamEditorCrew, setTeamEditorCrew] = useState(null);
  const [teamEditorText, setTeamEditorText] = useState("");
  const [analyticsMonth, setAnalyticsMonth] = useState(ymd(today).slice(0, 7));
  ...
  const lookAheadTopRef = useRef(null);
  const lookAheadTableRef = useRef(null);
  ...
  async function installApp() {
    if (!installPrompt) {
      flash("Use your browser menu and choose Install App or Add to Home Screen.");
      return;
    }
  ...
  const selectedDateIsPast = selectedDate < ymd(today);
  ...
  const availableCrews = selectedDateIsPast ? [] : crews.filter((c) => c.active !== false && !busyCrewIds.has(String(c.id)));
  ...
  async function updateAssignmentAction(assignment, action) {
    ...
    if (supabase) {
      const { data, error } = await supabase.from("assignments").update({ status: newStatus, notes: updatedNotes }).eq("id", assignment.id).select().single();
      if (error) return flash(error.message);
      setAssignments(assignments.map(a => a.id === assignment.id ? data : a));
    } else {
      const next = assignments.map(a => a.id === assignment.id ? { ...a, status: newStatus, notes: updatedNotes } : a);
  ...
  function parseCrewMembers(value) {
    if (!value) return [];
  ...
  function openTeamEditor(crew) {
    const members = parseCrewMembers(crew.crew_members);

    setTeamEditorCrew(crew);
    setTeamEditorText(
      members.map(member => `${member.name || ""} | ${member.title || ""}`).join("\n")
    );
  }

  async function saveTeamMembers() {
    if (!teamEditorCrew) return;

    const members = teamEditorText
      .split("\n")
      .map(line => line.trim())
      .filter(Boolean)
      .map(line => {
    ...
    if (supabase) {
      const { data, error } = await supabase
        .from("crews")
        .update({ crew_members: members })
        .eq("id", teamEditorCrew.id)
        .select()
        .single();

      if (error) return flash(error.message);

      setCrews(crews.map(c => c.id === teamEditorCrew.id ? data : c));
    } else {
      const next = crews.map(c =>
        c.id === teamEditorCrew.id
          ? { ...c, crew_members: members }
          : c
      );
    ...
    setTeamEditorCrew(null);
    setTeamEditorText("");
    flash("Crew members updated.");
  }
  ...
  const superintendentFlowGroups = Object.entries(superintendentGroups)
    .map(([superintendent, crewList]) => ({
      superintendent,
      crews: [...crewList].sort((a, b) => {
        const aNum = parseInt(String(a.crew_number || "").replace(/[^0-9]/g, "")) || 9999;
        const bNum = parseInt(String(b.crew_number || "").replace(/[^0-9]/g, "")) || 9999;
        return aNum - bNum || a.name.localeCompare(b.name);
      })
    }))
    .sort((a, b) => a.superintendent.localeCompare(b.superintendent));

  const todayString = ymd(today);

  function rawAssignmentFor(crewId, date) {
    return assignments.find(a => String(a.crew_id) === String(crewId) && a.work_date === date);
  }
  ...
  function historyStatusLabel(a) {
    if (!a) return "No Record";
    if (["Crew Called Off", "No Work", "Cancelled", "Off"].includes(a.status)) return a.status;
    return a.status || "Worked";
  }

  function downloadLookAheadExcel() {
    const rows = [];
    for (const date of lookAheadDates) {
      for (const crew of crews.filter(c => c.active !== false)) {
        const a = rawAssignmentFor(crew.id, date);
        rows.push({
          Date: date,
          "Crew / Foreman": crew.name,
          "Crew Number": crew.crew_number || "",
          Superintendent: crew.superintendent || "",
          Status: a ? historyStatusLabel(a) : (isPastDate(date) ? "No Record" : "Available"),
          Project: a ? projectName(a.project_id) : "",
    ...
    worksheet["!cols"] = [{wch:13},{wch:24},{wch:14},{wch:22},{wch:18},{wch:16},{wch:22},{wch:22},{wch:20},{wch:38}];
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "2 Week Look Ahead");
    XLSX.writeFile(workbook, `LGC-2-Week-Look-Ahead-${lookAheadDates[0]}-to-${lookAheadDates[lookAheadDates.length-1]}.xlsx`);
  }

  const workedStatuses = ["Assigned", "Tentative"];
  const analyticsAssignments = assignments.filter(a => a.work_date?.startsWith(analyticsMonth) && a.crew_id && workedStatuses.includes(a.status));

  const analyticsProjectMap = {};
  analyticsAssignments.forEach(a => {
    const project = projectName(a.project_id);
    if (!analyticsProjectMap[project]) analyticsProjectMap[project] = { project, crewIds: new Set(), crewDays: 0 };
    analyticsProjectMap[project].crewIds.add(String(a.crew_id));
    analyticsProjectMap[project].crewDays += 1;
  });
  const projectAnalytics = Object.values(analyticsProjectMap).map(x => ({project:x.project, crews:x.crewIds.size, crewDays:x.crewDays})).sort((a,b)=>b.crewDays-a.crewDays);

  const crewProjectMap = {};
  analyticsAssignments.forEach(a => {
    const crew = crewName(a.crew_id), project = projectName(a.project_id), key = `${crew}||${project}`;
    if (!crewProjectMap[key]) crewProjectMap[key] = {crew, project, days:0};
    crewProjectMap[key].days += 1;
  });
  const crewProjectAnalytics = Object.values(crewProjectMap).sort((a,b)=>b.days-a.days);

  const crewTotalsMap = {};
  analyticsAssignments.forEach(a => { const crew = crewName(a.crew_id); crewTotalsMap[crew] = (crewTotalsMap[crew] || 0) + 1; });
  const crewTotalsAnalytics = Object.entries(crewTotalsMap).map(([crew,days])=>({crew,days})).sort((a,b)=>b.days-a.days);

  const totalWorkedDays = crewTotalsAnalytics.reduce((sum,x)=>sum+x.days,0);
  const activeProjectsInMonth = projectAnalytics.length;
  const activeCrewsInMonth = crewTotalsAnalytics.length;
  const maxProjectDays = Math.max(1, ...projectAnalytics.map(x=>x.crewDays));
  const maxCrewDays = Math.max(1, ...crewTotalsAnalytics.map(x=>x.days));
  const piePalette = ["#246bdc", "#2ca35a", "#7446cf", "#d88919", "#14a3a8", "#c94f7c", "#6c7a89"];
  let pieStart = 0;
  const pieSegments = projectAnalytics.map((item,index) => {
    const share = totalWorkedDays ? item.crewDays / totalWorkedDays * 100 : 0;
    const segment = {...item, share, start:pieStart, end:pieStart+share, color:piePalette[index % piePalette.length]};
    pieStart += share;
    return segment;
  });
  const pieGradient = pieSegments.length ? `conic-gradient(${pieSegments.map(x=>`${x.color} ${x.start}% ${x.end}%`).join(", ")})` : "conic-gradient(#e8edf3 0 100%)";


  function quickAssignFromLookAhead(crew, date) {
    setCrewId(String(crew.id));
    setStartDate(date);
    setEndDate(date);
    setStatus("Assigned");
    setActiveTab("assign");
  ...
  function syncLookAheadScroll(source) {
    const top = lookAheadTopRef.current;
    const table = lookAheadTableRef.current;
    if (!top || !table) return;
      ...
      <div className="appShell">
        <aside className="sideNav">
          <div className="sideBrand">
            <img src="/lgc-global-logo.png" alt="LGC Global" />
          </div>

          <div className="sideMenu">
            <button className={activeTab==="dashboard" ? "active" : ""} onClick={()=>setActiveTab("dashboard")}>
              <span className="sideIcon">⌂</span><span>Dashboard</span>
            </button>
            <button className={activeTab==="assign" ? "active" : ""} onClick={()=>setActiveTab("assign")}>
              <span className="sideIcon">＋</span><span>Crew Planning</span>
            </button>
            <button className={activeTab==="dashboard" ? "lookAheadActive" : ""} onClick={()=>{setActiveTab("dashboard"); setTimeout(()=>document.getElementById("look-ahead")?.scrollIntoView({behavior:"smooth"}),50)}}>
              <span className="sideIcon">▦</span><span>2-Week Look Ahead</span>
            </button>
            <button className={activeTab==="requests" ? "active" : ""} onClick={()=>setActiveTab("requests")}>
              <span className="sideIcon">◎</span><span>Crew Requests</span>
              {groupedCrewRequests.length>0 && <span className="navCount">{groupedCrewRequests.length}</span>}
            </button>
            <button className={activeTab==="availability" ? "active" : ""} onClick={()=>setActiveTab("availability")}>
              <span className="sideIcon">✓</span><span>Availability</span>
            </button>
            <button className={activeTab==="projects" ? "active" : ""} onClick={()=>setActiveTab("projects")}>
              <span className="sideIcon">▣</span><span>Projects</span>
            </button>
            <button className={activeTab==="crews" ? "active" : ""} onClick={()=>setActiveTab("crews")}>
              <span className="sideIcon">♙</span><span>Crews</span>
            </button>
            <button className={activeTab==="reports" ? "active" : ""} onClick={()=>setActiveTab("reports")}>
              <span className="sideIcon">▤</span><span>Reports</span>
            </button>

            <div className="sideSectionTitle">CREW ANALYTICS</div>

            <button className={activeTab==="analytics" ? "active" : ""} onClick={()=>setActiveTab("analytics")}>
              <span className="sideIcon">▥</span><span>Crew Analytics</span>
            </button>
            <button className={activeTab==="flow" ? "active" : ""} onClick={()=>setActiveTab("flow")}>
              <span className="sideIcon">⌘</span><span>Crew Flow</span>
            </button>
          </div>

          <div className="sideBottom">
            <button className="sideInstall" onClick={installApp}>
              <span className="sideIcon">⇩</span><span>Install App</span>
            </button>
            <div className="sideVersion">LGC Crew Portal</div>
          </div>
        </aside>

        <div className="appMain">
          <header className="portalHeader">
            <div className="portalHeaderTitle">CREW PLANNING PORTAL</div>
            <div className="portalHeaderActions">
              <div className={`mode ${dbMode ? "live" : ""}`}>{dbMode ? "Shared Database" : "Local Mode"}</div>
              <button className="logoutBtn portalLogout" onClick={logout}>Logout</button>
            </div>
          </header>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="stats">
              ...
              <div className="stat green"><span>{selectedDateIsPast ? "Availability" : "Available"}</span><strong>{selectedDateIsPast ? "—" : availableCrews.length}</strong></div>
              <div className="stat red"><span>{selectedDateIsPast ? "Worked / Scheduled" : "Assigned / Reserved"}</span><strong>{busyCrewIds.size}</strong></div>
              <div className="stat"><span>Entries on Date</span><strong>{dateAssignments.length}</strong></div>
            </div>
            ...
            <div className="card lookAheadCard" id="look-ahead">
              <div className="lookAheadHead">
                <div>
                  <div className="cardTitle">2-Week Look Ahead</div>
                  <div className="cardSub">Project, work type, location, and schedule history for the next 14 days.</div>
                </div>
                <div className="lookAheadActions">
                  <button className="excelBtn compactExcelBtn" type="button" onClick={downloadLookAheadExcel}>Download Excel</button>
                  <span className="scrollHint">Scroll right →</span>
                </div>
              </div>
              <div
                className="lookAheadTopScroll"
                ref={lookAheadTopRef}
                onScroll={()=>syncLookAheadScroll("top")}
                aria-label="Scroll 2-week schedule horizontally"
              >
                <div className="lookAheadTopScrollInner"></div>
              </div>

              <div
                className="tableWrap twoWeekWrap"
                ref={lookAheadTableRef}
                onScroll={()=>syncLookAheadScroll("table")}
              >
                  ...
                  <tbody>
                    {crews.filter(c=>c.active!==false).map(c => (
                      <tr key={c.id}>
                        ...
                        {lookAheadDates.map((date) => {
                          const raw = rawAssignmentFor(c.id, date);
                          const past = isPastDate(date);
                          const calledOff = raw?.status === "Crew Called Off";
                          const noWork = raw && ["No Work","Cancelled","Off"].includes(raw.status);
                          const assigned = raw && !calledOff && !noWork;

                          return (
                            <td
                              key={date}
                              className={`${assigned ? "lookBusyCell" : ""} ${calledOff ? "calledOffCell" : ""} ${noWork ? "noWorkCell" : ""} ${past ? "pastDateCell" : ""} ${isWeekendDate(date) ? "weekendCell" : ""}`}
                            >
                              {calledOff ? (
                                <div className="lookCell calledOffHistory">
                                  <div className="availableLine">● Available</div>
                                  <small className="calledOffNote">Crew called off</small>
                                  {raw.project_id && <small className="historyProject">{projectName(raw.project_id)}</small>}
                                  <button className="pencilBtn" onClick={()=>setEditingAssignment(raw)} title="Edit history">✎</button>
                                </div>
                              ) : noWork ? (
                                <div className="lookCell offCellContent">
                                  <b>{historyStatusLabel(raw)}</b>
                                  {raw.notes && <small>{raw.notes.split(".")[0]}</small>}
                                  {raw.project_id && <small className="historyProject">{projectName(raw.project_id)}</small>}
                                  <button className="pencilBtn" onClick={()=>setEditingAssignment(raw)} title="Edit">✎</button>
                                </div>
                              ) : assigned ? (
                                <div className={`lookCell assignedLookCell ${past ? "historyCell" : ""}`}>
                                  <b>{raw.status}</b>
                                  {raw.project_id && <span>{projectName(raw.project_id)}</span>}
                                  {raw.work_type && <small className="workTypeText">{raw.work_type}</small>}
                                  {raw.location && <small>{raw.location}</small>}
                                  <button className="pencilBtn" onClick={()=>setEditingAssignment(raw)} title="Edit assignment">✎</button>
                                </div>
                              ) : past ? (
                                <div className="pastNoRecord"><span>—</span><small>No record</small></div>
                              ) : (
                                <div className="lookCell futureAvailable">
                                  <div className="availableLine">● Available</div>
                                  <button
                                    className="pencilBtn subtlePencil"
                                    onClick={()=>quickAssignFromLookAhead(c,date)}
                                    title={`Assign ${c.name} on ${date}`}
                                  >
                                    ✎
                                  </button>
                                </div>
            ...
            <div className="grid2">
              <div className="card">
                <div className="cardTitle">{selectedDateIsPast ? "Past Date" : "Available Crews"}</div>
                {selectedDateIsPast ? (
                  <div className="pastAvailabilityNote">Availability is not shown for past dates. Use the assignments and 2-week history above to review what happened.</div>
                ...
                {dateAssignments.length === 0 ? (
                  ...
                  dateAssignments.map((a) => (
                    <div className="assignment" key={a.id}>
                      ...
                      <button className="editBtn" onClick={() => setEditingAssignment(a)}>{selectedDateIsPast ? "View / Edit" : "Edit"}</button>
                    </div>
                  ))
                )}
              </div>
          ...
          <>
            <div className="pageHead analyticsPageHead">
              <div>
                <h1>Crew Analytics</h1>
                <p>Monthly crew utilization and project staffing insights.</p>
              </div>
              <label className="monthPickerLabel">Month<input type="month" value={analyticsMonth} onChange={e=>setAnalyticsMonth(e.target.value)} /></label>
            </div>

            <div className="analyticsStats">
              <div className="analyticsStatCard"><span>Active Crews</span><strong>{activeCrewsInMonth}</strong><small>crews with worked days</small></div>
              <div className="analyticsStatCard"><span>Active Projects</span><strong>{activeProjectsInMonth}</strong><small>projects using crews</small></div>
              <div className="analyticsStatCard"><span>Total Crew-Days</span><strong>{totalWorkedDays}</strong><small>worked days across all crews</small></div>
              <div className="analyticsStatCard"><span>Top Crew</span><strong>{crewTotalsAnalytics[0]?.crew || "—"}</strong><small>{crewTotalsAnalytics[0] ? `${crewTotalsAnalytics[0].days} worked days` : "No data"}</small></div>
            </div>

            <div className="analyticsGrid">
              <div className="card analyticsCard">
                <div className="cardTitle">Project Crew Usage</div>
                <div className="cardSub">Crew-days used by each project this month.</div>
                {projectAnalytics.length === 0 ? <div className="empty">No worked assignments for this month.</div> : (
                  <div className="barChart">{projectAnalytics.map(item => (
                    <div className="barRow" key={item.project}>
                      <div className="barLabel"><strong>{item.project}</strong><span>{item.crews} crew{item.crews===1?"":"s"}</span></div>
                      <div className="barTrack"><div className="barFill projectBar" style={{width:`${Math.max(6,item.crewDays/maxProjectDays*100)}%`}}></div></div>
                      <div className="barValue">{item.crewDays} days</div>
                    </div>
                  ))}</div>
                )}
              </div>
              ...
              <div className="card analyticsCard">
                <div className="cardTitle">Project Share</div>
                <div className="cardSub">Share of all crew-days by project.</div>
                <div className="pieLayout">
                  <div className="donutChart" style={{background:pieGradient}}><div className="donutCenter"><strong>{totalWorkedDays}</strong><span>crew-days</span></div></div>
                  <div className="pieLegend">{pieSegments.length===0 ? <div className="empty miniEmpty">No data</div> : pieSegments.map(segment => (
                    <div className="pieLegendRow" key={segment.project}><span className="pieColor" style={{background:segment.color}}></span><div><strong>{segment.project}</strong><small>{segment.share.toFixed(0)}%</small></div></div>
                  ))}</div>
                </div>
              </div>
            </div>
            ...
            <div className="analyticsGrid">
              <div className="card analyticsCard">
                <div className="cardTitle">Crew Worked Days</div>
                <div className="cardSub">Total worked days by crew this month.</div>
                {crewTotalsAnalytics.length===0 ? <div className="empty">No crew work recorded.</div> : (
                  <div className="barChart">{crewTotalsAnalytics.map(item => (
                    <div className="barRow" key={item.crew}>
                      <div className="barLabel"><strong>{item.crew}</strong><span>Worked</span></div>
                      <div className="barTrack"><div className="barFill crewBar" style={{width:`${Math.max(6,item.days/maxCrewDays*100)}%`}}></div></div>
                      <div className="barValue">{item.days} days</div>
                    </div>
                  ))}</div>
                )}
              </div>
              ...
              <div className="card analyticsCard">
                <div className="cardTitle">Crew × Project</div>
                <div className="cardSub">Where each crew spent its time.</div>
                {crewProjectAnalytics.length===0 ? <div className="empty">No crew/project data recorded.</div> : (
                  <div className="insightTableWrap"><table className="insightTable"><thead><tr><th>Crew</th><th>Project</th><th>Days</th></tr></thead><tbody>
                    {crewProjectAnalytics.map((item,index)=><tr key={`${item.crew}-${item.project}-${index}`}><td><strong>{item.crew}</strong></td><td>{item.project}</td><td><span className="daysPill">{item.days}</span></td></tr>)}
                  </tbody></table></div>
                )}
              </div>
            </div>

            <div className="analyticsInsightStrip"><strong>Management tip:</strong> Spot overloaded projects, under-used crews, and how many crew-days each project is consuming.</div>
          </>
        ) : (
          <>
            <div className="pageHead flowPageHead">
              <div>
                <h1>Crew Flow Chart</h1>
                <p>Superintendent → crew/foreman → crew members.</p>
              </div>
            </div>

            <div className="card flowChartCard">
              <div className="flowLegend">
                <div><span className="legendDot legendSup"></span>Superintendent</div>
                <div><span className="legendDot legendCrew"></span>Foreman / Crew</div>
                <div><span className="legendDot legendMember"></span>Crew Members</div>
              </div>

              <div className="orgChart">
                {superintendentFlowGroups.map(group => (
                  <div className="orgGroup" key={group.superintendent}>
                    <div className="orgSuperintendent">
                      <div className="orgSupIcon">●</div>
                      <div className="orgSupText">
                        <small>SUPERINTENDENT</small>
                        <strong>{group.superintendent}</strong>
                        <span>{group.crews.length} crew{group.crews.length===1 ? "" : "s"}</span>
                      </div>
                    </div>

                    <div className="orgBranchColumn">
                      {group.crews.map(crew => {
                        const members = parseCrewMembers(crew.crew_members);

                        return (
                          <div className="orgCrewRow" key={crew.id}>
                            <div className="orgConnector">
                              <span className="orgConnectorLine"></span>
                              <span className="orgArrow">→</span>
                            </div>

                            <div className="orgCrewCard">
                              <div className="orgCrewMain">
                                <div className="crewCardLeft">
                                  <div className="crewNumberBadge">{crew.crew_number || "Crew"}</div>
                                  <div className="crewCardText">
                                ...
                                <button
                                  className="teamEditBtn"
                                  onClick={() => openTeamEditor(crew)}
                                >
                                  ✎ Edit Team
                                </button>
                              </div>
                            ...
                            <div className="orgConnector memberConnector">
                              <span className="orgConnectorLine"></span>
                              <span className="orgArrow">→</span>
                            </div>
                            ...
                            <div className="orgMemberCard">
                              {members.length === 0 ? (
                                <div className="noMembers">
                                  <div className="noMemberIcon">●●●</div>
                                  <div className="noMemberText">
                                    <strong>No crew members added</strong>
                                    <span>Use Edit Team to add names and titles.</span>
                                  </div>
                                </div>
                              ) : (
                                <div className="memberList">
                                  {members.map((member, index) => (
                                    <div className="memberRow" key={`${crew.id}-${index}`}>
                                      <div className="memberNameWrap">
                                        <span className="memberIcon">●</span>
                                        <strong>{member.name}</strong>
                                      </div>
                                      <span className="memberRoleBadge">{member.title || "Crew Member"}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
              ...
              <div className="flowChartNote">
                <span>→</span>
                Arrows show the reporting structure.
              </div>
            </div>

            {teamEditorCrew && (
              <div className="editModalBackdrop" onClick={() => setTeamEditorCrew(null)}>
                <div className="editModal teamEditorModal" onClick={e => e.stopPropagation()}>
                  <div className="editModalHead">
                    <div>
                      <div className="eyebrow">FLOW CHART ONLY</div>
                      <h2>{teamEditorCrew.name}</h2>
                      <p>{teamEditorCrew.crew_number || "Crew"} • Crew members and titles</p>
                    </div>

                    <button
                      className="modalClose"
                      onClick={() => setTeamEditorCrew(null)}
                    >
                      ×
                    </button>
                  </div>

                  <label className="teamEditorLabel">
                    Crew Members
                    <textarea
                      rows="9"
                      value={teamEditorText}
                      onChange={e => setTeamEditorText(e.target.value)}
                      placeholder={"Devin Devine | Operator\nMatt Scherer | Laborer\nRoss Terry | Laborer"}
                    />
                  </label>

                  <div className="teamEditorHelp">
                    One person per line:
                    <strong>Name | Title</strong>
                  </div>

                  <div className="buttonRow">
                    <button className="primary" onClick={saveTeamMembers}>
                      Save Team
                    </button>
                    <button className="secondary" onClick={() => setTeamEditorCrew(null)}>
                      Cancel
                    </button>
                  </div>
                </div>
