      ...
      <section className="container">
        ...
        {loading ? (
                  ...
                  <thead>
                    <tr>
                      ...
                      {lookAheadDates.map((date) => (
                        <th key={date}>{fmt(new Date(date + "T12:00:00"))}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {crews.filter(c=>c.active!==false).map(c => (
                      <tr key={c.id}>
                        ...
                        {lookAheadDates.map((date) => {
                          ...
                          return (
                            <td key={date} className={a ? "lookBusyCell" : ""}>
                              {a ? (
          ...
          <>
            ...
            <div className="card">
              <div className="lookAheadHead"><div><div className="cardTitle">Open Crew Requests</div><div className="cardSub">Requests are shown separately from assigned crews.</div></div><span className="requestBadge">{groupedCrewRequests.length} Open</span></div>
          ...
          <>
            ...
            <div className="grid2">
              ...
              <div className="card">
                ...
                {crews.map(c=><div className="crewRow manageRow" key={c.id}><div className="dot"></div><div><strong>{c.name}</strong><small>{c.specialty || "General"}</small></div><button className="editBtn" onClick={()=>editCrew(c)}>Edit</button><button className="delete" onClick={()=>deleteCrew(c.id)}>Delete</button></div>)}
              </div>
            </div>
          </>
        ) : activeTab === "projects" ? (
